...
def party_new(request):
    form = PartyForm()
    return render(request, 'PartyCreate', {'form': form})
...
def party_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            party.createdBy = request.user
            party.save()
            return redirect('party_screen.html', pk=party.pk)
    else:
...
def party_detail(request, pk):
    ...
    return render(request, 'Party', {'Party': party})
...
def login(request):
    ...
    if form.is_valid():
        ...
        return render(request, 'Map', {'Party': party})